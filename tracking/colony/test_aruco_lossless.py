...
def test_end_to_end_mapping_and_tracking_cli_with_mixed_duplicates(tmp_path, input_format):
    ...
    assert complete == ["000"]
    # Synthetic calibration has two arenas separated at X=2000; supply its
    # annotations explicitly instead of depending on a production default.
    (tmp_path / "panorama_regions.csv").write_text(
        "semantic_label,shape,name,tracking_x_min_px,tracking_x_max_px\n"
        "arena_L,rectangle,left,0,1900\n"
        "arena_R,rectangle,right,2100,4000\n"
    )
