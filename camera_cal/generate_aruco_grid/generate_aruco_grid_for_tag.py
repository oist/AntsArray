...
def generate_aruco_grid(dictionary_name, rows, cols, tag_size=100, margin=10, output_file='aruco_grid.png', start_id=0, end_id=None):
    # Load the dictionary
    ...
    for r in range(rows):
        for c in range(cols):
            tag_id = start_id + r * cols + c
            
            if end_id is not None and tag_id > end_id:
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start-id', type=int, default=0, help='First tag ID to generate (default: 0)')
    parser.add_argument('--end-id', type=int, default=None, help='Last tag ID to generate (optional)')
    ...
    # Generate the grid
    generate_aruco_grid(aruco_dict, args.rows, args.cols, args.tag_size, args.margin, args.output_file, args.start_id, args.end_id)
