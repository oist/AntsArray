...
def combine_sleap(H_mats, curr_dir,output_folder_path, output_file_name):
    ...
    csv_files = sorted(glob.glob(curr_dir + '*.csv'))
    filtered_files = [file for file in glob.glob(curr_dir + '*.csv') if 'frame_counts' not in file and 'global' not in file]
